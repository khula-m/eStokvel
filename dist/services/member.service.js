...
class MemberService {
    async addMember(data, invitedById) {
        ...
        const roleValue = data.role && Object.values(client_1.MemberRole).includes(data.role)
            ? data.role
            : client_1.MemberRole.MEMBER;
    ...
    async updateMember(id, data) {
        ...
        const roleValue = role && Object.values(client_1.MemberRole).includes(role)
            ? role
            : undefined;
        if (role && !roleValue) {
    ...
    async removeMember(id, removedById) {
        const member = await prisma.member.findUnique({
            where: { id },
            include: {
                user: true,
    ...
    async joinGroupWithCode(userId, code) {
        const group = await prisma.stokvelGroup.findUnique({
            where: { code },
            include: {
                _count: {
